    ...
    { 
        if (isset($_POST['submit']))
        {
            if (is_numeric($_POST['ID']))
            {
                ...
                {
                    if ($stmt = $mysqli->prepare("UPDATE agenda SET StartTime = ?, EndTime = ?, ActivityDetails = ?, SpeakerID = ? WHERE ID='" . $ID . "'"))
                    {
                        $stmt->bind_param("sssi", $StartTime, $EndTime, $ActivityDetails, $SpeakerID);
                        $stmt->execute();
                        $stmt->close();
                        header("Location: agenda.php?EventID=" . $_GET['EventID']);  
                    }
                    else
                    {
...
<body>
    ...
    <div class="main-content">
        <div class="container-path">
            <p><span class="container-path-pages">Pages /</span> Modify agenda</p>
        </div>
                                            ...
                                            ?>
                                                <div class="event-information label">
                                                    <label>Start time</label>
                                                        <input type="time" name="StartTime" placeholder="Type start time" required value="<?php echo $row->StartTime;?>">
                                                    <label>End time</label>
                                                            <input type="time" name="EndTime" placeholder="Type end time" required value="<?php echo $row->EndTime;?>">
                                                    <label>Activity details</label>
                                                        <input type="text" name="ActivityDetails" placeholder="Type activity details" required value="<?php echo $row->ActivityDetails;?>">
                                                    <label>Speaker</label>
                                                        <?php
                                                            include("../../src/conectare.php");
