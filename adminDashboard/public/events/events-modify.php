    ...
    { 
        if (isset($_POST['submit']))
        {
            if (is_numeric($_POST['ID']))
            {
                ...
                {
                    if ($stmt = $mysqli->prepare("UPDATE event SET Name = ?, Date = ?, Location = ?, Tickets = ?, ContactName = ?, ContactPhone = ?, ContactEmail = ? WHERE ID='" . $ID . "'"))
                    {
                    ...
                    <form id="myForm" class="form" action="" method="POST">
                        <?php 
                            ...
                            { 
                                ...
                                <?php
                                    if ($result = $mysqli->query("SELECT * FROM event where ID = '" . $_GET['ID'] . "'"))
                                    {
                                            ...
                                            ?>
                                                <div class="event-information label">
                                                    <label>Event name</label>
                                                        <input type="text" name="Name" placeholder="Type event name" required value="<?php echo $row->Name;?>">
                                                    <label>Event date</label>
                                                            <input type="date" name="Date" placeholder="Type event date" required value="<?php echo $row->Date;?>"> 
                                                    <label>Event location</label>
                                                        <input type="text" name="Location" placeholder="Type event location" required value="<?php echo $row->Location;?>">
                                                    <label>NO tickets</label>
                                                        <input type="number" name="Tickets" placeholder="Type number of tickets" required value="<?php echo $row->Tickets;?>">
                                                    <label>Contact name</label>
                                                        <input type="text" name="ContactName" placeholder="Type contact name" required value="<?php echo $row->ContactName;?>">
                                                    <label>Contact phone</label>
                                                            <input type="tel" name="ContactPhone" placeholder="Type contact phone" required value="<?php echo $row->ContactPhone;?>">
                                                    <label>Contact email</label>
                                                        <input type="email" name="ContactEmail" placeholder="Type contact email" required value="<?php echo $row->ContactEmail;?>">
                                                </div>
                                                <?php
                                        }
                                    }
